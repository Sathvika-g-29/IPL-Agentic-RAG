from pathlib import Path
import json

from src.embeddings import tokenize
from src.state import IPLState


CONFLICTS_PATH = Path("data/validation_conflicts.json")


def load_conflicts() -> list[dict]:
    if not CONFLICTS_PATH.exists():
        return []

    return json.loads(CONFLICTS_PATH.read_text(encoding="utf-8"))


def validation_node(state: IPLState) -> IPLState:
    query_words = set(tokenize(state["query"]))
    matched_conflicts = []

    for conflict in load_conflicts():
        fact_words = set(tokenize(conflict["fact"]))

        if len(query_words.intersection(fact_words)) >= 2:
            matched_conflicts.append(conflict)

    state["conflict_detected"] = bool(matched_conflicts)
    state["conflicts"] = matched_conflicts

    if matched_conflicts:
        conflict_lines = []
        for conflict in matched_conflicts:
            conflict_lines.append(
                f"Conflict detected for {conflict['fact']}: "
                f"primary source says {conflict['primary_value']}, "
                f"secondary source says {conflict['secondary_value']}. "
                f"{conflict['expected_behavior']}."
            )

        state["answer"] = "\n".join(conflict_lines)

    return state
